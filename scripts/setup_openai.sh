...
if [ -z "$1" ]; then
    ...
    echo "Usage: ./setup_openai.sh YOUR_OPENAI_API_KEY"
    echo ""
    echo "Example:"
    echo "  ./setup_openai.sh sk-1234567890abcdef..."
    exit 1
fi
...
if [ $? -eq 0 ]; then
    ...
    echo "  - 'What is my deductible?'"
    echo "  - 'Can I see a specialist?'"
    echo "  - 'What's covered for mental health?'"
else
    echo ""
